...
export const PolishingRoSystem = ({ position, colorPalette, blueprint, onSelect, flowSpeed, telemetryData = {} }) => {
 if (!blueprint) return null;

 // Process specific pipe colors if any mapping is needed
 const mappedPipes = blueprint.pipes?.map(pipe => {
 let newColor = pipe.color;
 if (pipe.color === 'WHITE') newColor = colorPalette?.processPipe || '#e2e8f0';
 if (pipe.color === 'BLUE') newColor = colorPalette?.permeate || '#06b6d4';
 if (pipe.color === 'YELLOW') newColor = '#eab308'; // RO Reject
 if (pipe.color === 'RED') newColor = '#ef4444'; // CIP
